...
def getToken(salt_value):
    ...
    pwdaction = str(salt_value) + str(powSha1.hexdigest()) + action

    auth_sign = hashlib.sha1()
    auth_sign.update(pwdaction.encode('utf-8'))
    sign = str(auth_sign.hexdigest())

    solarurl = baseURL + '?sign=' + sign + '&salt=' + str(salt_value) + action
...
def buildRequestUrl(action, salt_value, secret, token, devcode, plantId, pn, sn):
  ...
  reqaction= str(salt_value) + secret + token + action
  req_sign=hashlib.sha1()
  req_sign.update(reqaction.encode('utf-8'))
  sign = str(req_sign.hexdigest())
  requrl= baseURL + '?sign='+ sign + '&salt=' + str(salt_value) + '&token=' + token + action
  return requrl
...
def getAllData():
  ...
  requrl = buildRequestUrl('queryPlantDeviceDesignatedInformation', salt(), secret, token, config.devcode, config.plantId, config.pn, config.sn)
  if config.debug == 1:
    print (requrl)
  r = requests.get(requrl)
  ...
  requrl = buildRequestUrl('queryPlantActiveOuputPowerOneDay', salt(), secret, token, config.devcode, config.plantId, config.pn, config.sn)
  if config.debug == 1:
    print (requrl)
  r = requests.get(requrl)
